...
from db import Base
from recipes.schemas import (AmountOut, FavoriteOut, QueryParams, RecipeOut,
                             SAmountIngredient, SIngredient, STag, TagOut)
...
class Amount_ingredient(Base):
    __tablename__ = "amount_ingredient"
    __table_args__ = (
...
class TagDB:
    ...
    async def get_tags_by_recipe_id(session: AsyncSession, pk: int) -> list[TagOut]:
        tags_dict = await session.execute(
            ...
            .join(Recipe_tag, Recipe_tag.tag_id == Tag.id)
            .where(Recipe_tag.recipe_id == pk)
            .order_by(Tag.id)
        )
...
class AmountDB:
    async def get_amount_by_recipe_id(session: AsyncSession, pk: int) -> list[AmountOut]:
        amount_dict = await session.execute(
            ...
            .join(Ingredient, Amount_ingredient.ingredient_id == Ingredient.id)
            .where(Amount_ingredient.recipe_id == pk)
        )
...
class RecipeDB:
    async def _create_recipe_tag(session: AsyncSession, recipe_id: int, tags: list) -> Any:
        ...
        return await session.execute(sa.insert(Recipe_tag).values(tags))

    async def _create_amount_ingredient(session: AsyncSession, recipe_id: int, ingredients: list) -> Any:
        result = [
            {
                "recipe_id": recipe_id,
                "ingredient_id": i["id"],
        ...
        return await session.execute(sa.insert(Amount_ingredient).values(result))
    ...
    async def check_author_by_id(session: AsyncSession, pk: int) -> int:
        query = await session.execute(
            sa.select(Recipe.author_id).where(Recipe.id == pk)
        )
        return query.scalar()

    async def check_recipe_author_image_by_id(session: AsyncSession, pk: int) -> Any:
        query = await session.execute(
    ...
    async def check_recipe_by_id_author(
        ...
        if author_id:
            query = (
                query.where(Recipe.author_id == author_id)
    ...
    async def count_recipe(
        ...
        if tags and tags.tags:
            query = (
                query.join(Recipe_tag, Recipe_tag.recipe_id == Recipe.id)
                .join(Tag, Recipe_tag.tag_id == Tag.id)
    ...
    async def get(
        ...
        if pk:
            ...
            if tags and tags.tags:
                query = (
                    query
                    .join(Recipe_tag, Recipe_tag.recipe_id == Recipe.id, isouter=True)
                    .join(Tag, Recipe_tag.tag_id == Tag.id, isouter=True)
                    .where(Tag.slug.in_(tags.tags))
    ...
    async def update(
        ...
        try:
            ...
            await session.execute(
                sa.delete(Recipe_tag)
                .where(Recipe_tag.recipe_id == pk)
            )
            await RecipeDB._create_recipe_tag(session, recipe_id, tags)

            await session.execute(
                sa.delete(Amount_ingredient)
                .where(Amount_ingredient.recipe_id == pk)
            )
            await RecipeDB._create_amount_ingredient(session, recipe_id, ingredients)
            await session.commit()
    ...
    async def delete(session: AsyncSession, pk: int) -> None:
        await session.execute(
            sa.delete(Recipe).where(Recipe.id == pk)
        )
        await session.commit()
...
class FavoriteCartDB:
    async def get_shopping_cart(session: AsyncSession, user_id: int) -> list:
        query = await session.execute(
            sa.select(
                Ingredient.name,
                Ingredient.measurement_unit,
                func.sum(Amount_ingredient.amount).label("amount"),
            )
            .filter(Cart.recipe_id == Amount_ingredient.recipe_id)
            .join(Ingredient, Amount_ingredient.ingredient_id == Ingredient.id)
    ...
    async def create(session: AsyncSession, recipe_id: int, user_id: int, db_model: Any) -> bool:
        try:
            await session.execute(
                sa.insert(db_model).values(recipe_id=recipe_id, user_id=user_id)
            )
            await session.commit()
            return True
    ...
    async def delete(session: AsyncSession, recipe_id: int, user_id: int, db_model: Any) -> bool:
        try:
            await session.execute(
