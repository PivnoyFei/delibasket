...
async def get_tags(params: SearchName = Depends()) -> dict:
    """Cписок тегов."""
    return await tag.get_all_list(
        params,
        attr_name="name",
        query=Tag.list_columns("id", "name", "color", "slug"),
    )
...
async def details_tag(tag_id: int) -> Any:
    """Получение тега."""
    return await tag.by_id(pk=tag_id) or NOT_FOUND


@admin_router.patch("/{tag_id}/", response_model=TagOut, status_code=HTTP_200_OK)
async def update_tag(tag_id: int, items: TagUpdate) -> JSONResponse:
...
async def delete_tag(tag_id: int) -> JSONResponse:
    if not await tag.delete(pk=tag_id):
        return JSONResponse({"detail": "Тег не существует"}, HTTP_404_NOT_FOUND)
