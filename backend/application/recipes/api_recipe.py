...
from application.recipes.models import (
    Cart,
    Favorite,
    FavoriteCartDB,
    IngredientDB,
...
from application.recipes.utils import (
    base64_image,
    create_cart_favorite,
    delete_cart_favorite,
    tag_ingredient,
)
from application.services import image_delete, query_list
from application.settings import FILES_ROOT, NOT_AUTHENTICATED, NOT_FOUND
...
async def create_tag(
    tag: STag, user: User = PROTECTED, session: AsyncSession = SESSION
) -> JSONResponse:
...
@router.delete("/tags/{pk}/", status_code=status.HTTP_200_OK)
...
async def create_recipe(
    ...
    try:
        ...
        pk = await RecipeDB.create(session, recipe_item, recipe.ingredients, recipe.tags)
        ...
        return JSONResponse({"detail": "Error recipe"}, status.HTTP_400_BAD_REQUEST)


@router.get("/recipes/download_shopping_cart/", response_model=None, status_code=status.HTTP_200_OK)
async def download_shopping_cart(
    ...
    if not ingredients:
        return JSONResponse({"detail": "No recipes in cart"})
...
@router.delete("/recipes/{pk}/", status_code=status.HTTP_200_OK)
async def recipes(
    request: Request,
    pk: int | None = None,
    ...
    if pk:
        recipe_dict = await RecipeDB.get(session, request, pk=pk, user_id=user_id)
    ...
    if recipe_dict:
        count = await RecipeDB.count_recipe(session, author, tags, is_favorited, is_in_cart)
...
async def update_recipe(
    ...
    if author.author_id == user.id or user.is_staff or user.is_superuser:
        ...
        try:
            ...
            return JSONResponse({"detail": "error"}, status.HTTP_400_BAD_REQUEST)

    return Response(status_code=status.HTTP_403_FORBIDDEN)


@router.post("/recipes/{pk}/favorite/", response_model=FavoriteOut, status_code=status.HTTP_200_OK)
@router.delete("/recipes/{pk}/favorite/", response_model=None, status_code=status.HTTP_200_OK)
@router.post(
    "/recipes/{pk}/shopping_cart/", response_model=FavoriteOut, status_code=status.HTTP_200_OK
)
@router.delete("/recipes/{pk}/shopping_cart/", response_model=None, status_code=status.HTTP_200_OK)
